...
export const CategoryForm: React.FC<CategoryFormProps> = ({ initialData, banners }) => {
  ...
  const onSubmit = async (data: CategoryFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.hotelId}/categories/${params.categoryId}`, data);
      } else {
        await axios.post(`/api/${params.hotelId}/categories`, data);
      }
      router.refresh();
      router.push(`/${params.hotelId}/categories`);
      toast.success(toastMessage);
    } catch (error) {
      toast.error("Check your data input");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.hotelId}/categories/${params.categoryId}`);
      router.refresh();
      router.push(`/${params.hotelId}/categories`);
